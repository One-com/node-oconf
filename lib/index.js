...
module.exports.load = function load(rootFileName) {
    var loadedAbsoluteFileNames = [];
            ...
            if (Array.isArray(obj)) {
                ...
                if (typeof obj['#include'] !== 'undefined') {
                    _.flatten([obj['#include']]).reverse().forEach(function (fileNameToInclude) {
                        obj = cjson.extend(true, readFileAndResolveIncludes(Path.resolve(Path.dirname(fileName), fileNameToInclude)), obj);
                    });
                    delete obj['#include'];
                }
